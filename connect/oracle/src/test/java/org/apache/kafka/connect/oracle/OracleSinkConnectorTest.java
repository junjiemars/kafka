...
public class OracleSinkConnectorTest {

    private static final String MULTIPLE_TOPICS = "edge";
    ...
    private static final String FILENAME = "/tmp/edge-sink.txt";

    private OracleSinkConnector connector;
    private ConnectorContext ctx;
    private Map<String, String> sinkProperties;

    @Before
    public void setup() {
        connector = new OracleSinkConnector();
        ctx = PowerMock.createMock(ConnectorContext.class);
        connector.initialize(ctx);

        sinkProperties = new HashMap<>();
        sinkProperties.put(SinkConnector.TOPICS_CONFIG, MULTIPLE_TOPICS);
        sinkProperties.put(OracleSinkConnector.FILE_CONFIG, FILENAME);
    }
    ...
    public void testSinkTasks() {
        ...
        assertEquals(FILENAME, taskConfigs.get(0).get(OracleSinkConnector.FILE_CONFIG));
    ...
    public void testTaskClass() {
        ...
        connector.start(sinkProperties);
        assertEquals(OracleSinkTask.class, connector.taskClass());
